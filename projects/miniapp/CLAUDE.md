...
| 登录/注册 | `pages/login/login` | 体验入口与单一维护者登录/注册入口；老用户直接登录，仅新用户授权手机号，可选推荐码；后端为新注册用户写入系统默认头像 |
...
| `precheckMaintainerWechatLogin(code)` | 预检当前微信身份下一步是否需要手机号授权 |
