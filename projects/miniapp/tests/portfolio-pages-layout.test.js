...
test('maintainer portfolio pages expose expected controls', () => {
  ...
  assert.match(listWxml, /wx:if="\{\{item\.actionType === 'SHARE'\}\}"[\s\S]*class="portfolio-icon-button share"[\s\S]*aria-label="分享作品集"[\s\S]*class="portfolio-share-icon"/)
  assert.match(listWxml, /class="portfolio-share-hit"[\s\S]*open-type="share"[\s\S]*data-id="\{\{item\.portfolioId\}\}"/)
  assert.match(listWxml, /class="portfolio-share-dot source"/)
  assert.match(listWxml, /class="portfolio-share-line upper"/)
  ...
  assert.match(portfolioActionButtonRule, /border-radius:\s*28rpx/)
  assert.match(portfolioActionButtonRule, /color:\s*#17202a/)
  assert.match(portfolioActionButtonRule, /border:\s*1rpx\s+solid\s+#cbd5df/)
  assert.match(portfolioActionButtonRule, /background:\s*#ffffff/)
  ...
  assert.match(listWxss, /\.portfolio-icon-button\s*\{[\s\S]*position:\s*relative;[\s\S]*width:\s*56rpx;[\s\S]*min-width:\s*56rpx;[\s\S]*max-width:\s*56rpx;[\s\S]*flex:\s*0 0 56rpx;[\s\S]*height:\s*56rpx;[\s\S]*overflow:\s*hidden;[\s\S]*border-radius:\s*50%;[\s\S]*background:\s*#f7fafc;/)
  assert.match(listWxss, /\.portfolio-share-hit\s*\{[\s\S]*position:\s*absolute;[\s\S]*left:\s*0;[\s\S]*top:\s*0;[\s\S]*width:\s*100%;[\s\S]*height:\s*100%;[\s\S]*opacity:\s*0;/)
  assert.match(listWxss, /\.portfolio-share-icon\s*\{[\s\S]*position:\s*relative;[\s\S]*width:\s*34rpx;[\s\S]*height:\s*34rpx;/)
  assert.match(listWxss, /\.portfolio-share-line\s*\{[\s\S]*position:\s*absolute;[\s\S]*height:\s*3rpx;[\s\S]*background:\s*currentColor;/)
...
test('standard personal portfolio editor follows shared maintainer layout', () => {
  ...
  assert.match(editWxml, /class="display-group-add-button"[\s\S]*catchtap="handleOpenCreateDisplayGroup"[\s\S]*新增标签/)
  assert.match(editWxml, /class="display-group-form \{\{displayGroupFormVisible \? 'visible' : ''\}\}"/)
  assert.match(editWxml, /bindinput="handleDisplayGroupNameInput"/)
  assert.match(editWxml, /catchtap="handleConfirmDisplayGroupForm"/)
  assert.match(editWxml, /bindlongpress="handleDisplayGroupLongPress"/)
  assert.match(editWxml, /class="display-group-delete"[\s\S]*catchtap="handleDeleteDisplayGroup"[\s\S]*<text class="display-group-delete-mark">×<\/text>/)
  assert.match(editWxml, /catchtap="handleDeleteDisplayGroup"/)
  ...
  assert.match(editWxss, /\.display-group-add-button\s*\{[\s\S]*height:\s*64rpx;/)
  assert.match(editWxss, /\.display-group-form\s*\{[\s\S]*overflow:\s*hidden;[\s\S]*max-height:\s*0;/)
  assert.match(editWxss, /\.display-group-form\.visible\s*\{[\s\S]*max-height:/)
  assert.match(editWxss, /\.display-group-pill\.manage\s*\{[\s\S]*padding-right:/)
  assert.match(editWxss, /\.display-group-delete\s*\{[\s\S]*border-radius:\s*50%;/)
  assert.match(editWxss, /\.display-group-delete-mark\s*\{[\s\S]*transform:\s*translateY\(-1rpx\);/)
