...
test('maintainer portfolio pages expose expected controls', () => {
  ...
  const portfolioShareButtonRule = readRule(listWxss, '.portfolio-share-button')
  const portfolioShareButtonDraftRule = readRule(listWxss, '.portfolio-share-button.draft')
  ...
  assert.match(portfolioShareButtonRule, /height:\s*58rpx/)
  assert.match(portfolioShareButtonRule, /border-radius:\s*12rpx/)
  assert.match(portfolioShareButtonRule, /color:\s*#17202a/)
  assert.match(portfolioShareButtonRule, /border:\s*1rpx\s+solid\s+rgba\(23,\s*32,\s*42,\s*0\.28\)/)
  assert.match(portfolioShareButtonRule, /background:\s*#ffffff/)
  assert.doesNotMatch(portfolioShareButtonRule, /color:\s*#ffffff/)
  assert.doesNotMatch(portfolioShareButtonRule, /background:\s*#17202a/)
  assert.doesNotMatch(portfolioShareButtonRule, /border-radius:\s*999rpx/)
  assert.doesNotMatch(portfolioShareButtonDraftRule, /background:\s*#fff4d7/)
  assert.doesNotMatch(portfolioShareButtonDraftRule, /border-color:\s*#eac265/)
...
test('portfolio list cards keep action buttons from squeezing title copy', () => {
  ...
  assert.match(listWxss, /\.portfolio-share-button\s*\{[\s\S]*width:\s*104rpx;[\s\S]*min-width:\s*104rpx;[\s\S]*max-width:\s*104rpx;[\s\S]*flex:\s*none;[\s\S]*box-sizing:\s*border-box;/)
})
...
test('standard personal portfolio editor follows shared maintainer layout', () => {
  ...
  assert.match(editWxml, /class="field-heading"[\s\S]*分享简介[\s\S]*\{\{shareFieldCounters\.intro\}\}/)
  ...
  assert.match(editWxml, /data-path="share\.intro"[\s\S]*maxlength="500"/)
