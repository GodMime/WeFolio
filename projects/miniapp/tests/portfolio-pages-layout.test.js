...
test('maintainer portfolio pages expose expected controls', () => {
  ...
  assert.match(editWxml, /\{\{item.sample\}\}/)
  assert.match(editWxml, /item.previewImageUrl/)
...
test('portfolio editor component rows keep order title drag handle and edit cue vertically centered', () => {
  ...
  const componentDragLineRule = readRule(editWxss, '.component-drag-line')
  ...
  assert.match(editWxml, componentDragHandleMarkup)
  assert.match(componentDragHandleRule, /display:\s*flex/)
  assert.match(componentDragHandleRule, /flex-direction:\s*column/)
  assert.match(componentDragHandleRule, /align-items:\s*center/)
  assert.match(componentDragLineRule, /width:\s*22rpx/)
  assert.match(componentDragLineRule, /height:\s*4rpx/)
  assert.doesNotMatch(componentDragHandleRule, /linear-gradient/)
...
test('standard personal portfolio editor follows shared maintainer layout', () => {
  ...
  assert.match(editWxss, /\.component-drag-handle\s*\{[\s\S]*display:\s*flex;[\s\S]*flex-direction:\s*column;/)
  assert.match(editWxss, /\.component-drag-line\s*\{[\s\S]*width:\s*22rpx;[\s\S]*height:\s*4rpx;/)
