...
test('team schedule query renders the personal-style month calendar grid', () => {
  ...
  const dayMetaRule = readRule(wxss, '.schedule-query-day-meta')
  const dayDotsRule = readRule(wxss, '.schedule-query-day-dots')
  const dayDotRule = readRule(wxss, '.schedule-query-day-dot')
  ...
  assert.match(wxml, /class="schedule-query-day-stack"[\s\S]*class="schedule-query-day-number"[\s\S]*class="schedule-query-day-meta" wx:if="\{\{item\.metaText\}\}"/)
  assert.match(wxml, /class="schedule-query-day-dots"[\s\S]*wx:for="\{\{item\.colors\}\}"[\s\S]*class="schedule-query-day-dot"/)
  ...
  assert.match(calendarDayRule, /justify-content:\s*space-between/)
