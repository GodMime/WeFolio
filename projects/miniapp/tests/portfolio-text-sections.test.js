...
for (const modulePath of MODULE_PATHS) {
  test(`${modulePath}: 编辑类型切换与安全展示`, async (t) => {
    ...
    await t.test('不提供 Object.hasOwn 的小程序运行时仍可规范化与校验', () => {
      const sandbox = { module: { exports: {} }, require: () => require('../utils/portfolio-text-typography') }
    ...
    await t.test('文字类型共用内容且所有独立样式在切换后保留', () => {
      const block = freeze({ ...rules.createStructuredBlock('TITLE', 'key'), content: 'A\n\n B ',
        fontSizeRpx: 72, lineHeight: 1.2, fontWeight: 'NORMAL', color: '#abcdef', alignment: 'RIGHT', marginBottomRpx: 124 })
      ...
      for (const field of ['blockKey', 'fontSizeRpx', 'lineHeight', 'fontWeight', 'color', 'alignment', 'marginTopRpx', 'marginBottomRpx']) {
    ...
    await t.test('留白只暂存原文，切回恢复；已保存留白转文字按新类型初始化', () => {
      let draft = rules.createBlockEditDraft({ ...rules.createStructuredBlock('TITLE', 't'), content: '原文', fontSizeRpx: 80, lineHeight: 2.3 })
      ...
      assert.equal(Object.hasOwn(draft.block, 'lineHeight'), false)
      assert.equal(draft.block.heightRpx, 32)
      assert.equal(draft.block.marginTopRpx, 0)
      assert.equal(draft.block.marginBottomRpx, 0)
      ...
      assert.equal(draft.block.lineHeight, 2.3)
      assert.equal(draft.block.marginTopRpx, 24)
      assert.equal(draft.block.marginBottomRpx, 16)
      ...
      assert.equal(Object.hasOwn(reopened.block, 'lineHeight'), false)
    ...
    await t.test('旧留白的间距在读取保存及重复选择留白类型时保持原值', () => {
      const block = freeze({ ...rules.createStructuredBlock('SPACER', 's'), marginTopRpx: 4, marginBottomRpx: 8 })
      const initial = freeze(rules.createBlockEditDraft(block))
      const result = rules.switchBlockEditType(initial, 'SPACER')
      assert.equal(result.error, '')
      assert.deepEqual(result.draft, initial)
      assert.notStrictEqual(result.draft, initial)
      assert.deepEqual(rules.finalizeStructuredTextConfig({ blocks: [result.draft.block] }).blocks[0], block)
      assert.match(rules.buildStructuredTextPresentation({ blocks: [block] }).blocks[0].style, /margin-top:4rpx;margin-bottom:8rpx;/)
    })

    await t.test('留白高度支持 0–512 rpx，保存展示不截断且文字间距仍限制 128 rpx', () => {
      const paragraph = { ...rules.createStructuredBlock('PARAGRAPH', 'p'), content: '正文' }
      for (const heightRpx of [0, 128, 132, 508, 512]) {
        const config = freeze({ blocks: [paragraph, { ...rules.createStructuredBlock('SPACER', 's'), heightRpx }] })
        assert.equal(rules.validateStructuredTextConfig(config), '')
        const saved = rules.finalizeStructuredTextConfig(config)
        assert.equal(saved.blocks[1].heightRpx, heightRpx)
        assert.match(rules.buildStructuredTextPresentation(saved).blocks[1].style, new RegExp(`height:${heightRpx}rpx;`))
      }
      for (const heightRpx of [-4, 3, 510, 516, '512', '512;display:none']) {
        const config = { blocks: [paragraph, { ...rules.createStructuredBlock('SPACER', 's'), heightRpx }] }
        assert.match(rules.validateStructuredTextConfig(config), /留白高度.*512/)
        assert.equal(rules.normalizeStructuredTextConfig(config).blocks[1].heightRpx, heightRpx)
        assert.match(rules.buildStructuredTextPresentation(config).blocks[1].style, /height:32rpx;/)
      }
      assert.match(rules.validateStructuredTextConfig({ blocks: [{ ...paragraph, marginTopRpx: 132 }] }), /区块间距.*128/)
      for (const [input, expected] of [[132, 132], [510, 512], [516, 512], [-4, 0]]) {
        assert.equal(rules.normalizeSpacingInput(input, rules.MAX_SPACER_HEIGHT), expected)
      }
      assert.equal(rules.normalizeSpacingInput(512), 128)
    })
    ...
    await t.test('背景按原比例计算最小高度，动图用原 URL，三种处理独立映射', () => {
      for (const [backgroundTreatment, treatmentClass] of [['ORIGINAL', 'text-background--original'],
        ...
        assert.deepEqual(rules.buildTextBackgroundPresentation(freeze({ backgroundEnabled: true, backgroundWorkId: 7,
          ...
          enabled: true, imageUrl: 'https://example.test/original.gif', isVideo: false, videoUrl: '', posterUrl: '', minHeightRpx: 1200, treatmentClass, invalid: false
        })
      }
    })

    await t.test('视频背景播放地址和封面分离，无封面仍可展示，保存只保留作品身份', () => {
      for (const posterUrl of ['https://example.test/poster.jpg', undefined]) {
        const config = freeze({ backgroundEnabled: true, backgroundWorkId: 7, backgroundMemberUserId: 9,
          backgroundTreatment: 'DARK_MASK', backgroundWork: { workId: 7, mediaType: 'VIDEO',
            url: 'https://example.test/video.mp4', posterUrl, width: 1920, height: 1080 } })
        assert.equal(rules.validateTextBackground(config, { team: true }), '')
        const background = rules.buildTextBackgroundPresentation(config)
        assert.equal(background.isVideo, true)
        assert.equal(background.videoUrl, config.backgroundWork.url)
        assert.equal(background.imageUrl, '')
        assert.equal(background.posterUrl, posterUrl || '')
        assert.equal(background.invalid, false)
        assert.equal(rules.buildTextBackgroundFrameStyle(background), 'min-height:421.875rpx;')
        assert.deepEqual(rules.finalizeTextBackground(config, { team: true }), {
          backgroundEnabled: true, backgroundWorkId: 7, backgroundMemberUserId: 9,
          backgroundTreatment: 'DARK_MASK', verticalAlignment: 'CENTER'
        })
      }
    })

    await t.test('背景选择缩略图仅取视频封面，动图保留原始动画且不修改源数据', () => {
      for (const [mediaType, url, posterUrl, thumbnailUrl, mediaTypeText] of [
        ['VIDEO', 'video.mp4', 'poster.jpg', 'poster.jpg', '视频'],
        ['VIDEO', 'video.mp4', undefined, '', '视频'],
        ['ANIMATION', 'animation.gif', 'poster.jpg', 'animation.gif', '动图'],
        ['IMAGE', 'image.jpg', undefined, 'image.jpg', '图片']
      ]) {
        const work = freeze({ id: 7, mediaType, url, posterUrl })
        assert.deepEqual(rules.buildTextBackgroundWorkPreview(work), {
          ...work, thumbnailUrl, mediaTypeText, isVideo: mediaType === 'VIDEO'
        })
      }
    })

    await t.test('视频元数据按实际容器宽度计算高度，切换资源后拒绝迟到回调', () => {
      const background = rules.buildTextBackgroundPresentation({ backgroundEnabled: true, backgroundWorkId: 7,
        backgroundWork: { workId: 7, mediaType: 'VIDEO', url: 'video.mp4' } })
      let measured, selector
      const component = { data: { background }, setData(patch) { Object.assign(this.data, patch) },
        createSelectorQuery() { return { select(value) { selector = value; return {
          boundingClientRect(callback) { measured = callback; return { exec() {} } }
        } } } } }
      const event = { currentTarget: { dataset: { src: 'video.mp4' } }, detail: { width: 1920, height: 1080 } }
      rules.handleTextBackgroundLoad(component, event)
      assert.equal(selector, '.text-background-video')
      assert.equal(component.data.frameStyle, 'min-height:421.875rpx;')
      measured({ width: 320 })
      assert.equal(component.data.frameStyle, 'min-height:180px;')
      assert.equal(rules.buildTextBackgroundFrameStyle(background, component._textBackgroundLayout), 'min-height:180px;')
      component.data.background = { ...background, videoUrl: 'next.mp4' }
      rules.handleTextBackgroundLoad(component, event)
      measured({ width: 640 })
      assert.equal(component.data.frameStyle, 'min-height:180px;')
    })
    ...
    await t.test('无效、关闭、无权 URL 和缺尺寸只产生安全展示数据', () => {
      ...
      for (const patch of [{}, { backgroundInvalid: true, backgroundWork: { workId: 7, mediaType: 'IMAGE', url: 'bad' } },
        ...
        { backgroundWork: { workId: 7, mediaType: 'AUDIO', url: 'audio.mp3' } },
        ...
        assert.deepEqual(rules.buildTextBackgroundPresentation({ ...base, ...patch }), {
          enabled: true, imageUrl: '', isVideo: false, videoUrl: '', posterUrl: '', minHeightRpx: 0, treatmentClass: 'text-background--gradient', invalid: true
        })
      }
      assert.deepEqual(rules.buildTextBackgroundPresentation({ ...base, backgroundEnabled: false }), {
        enabled: false, imageUrl: '', isVideo: false, videoUrl: '', posterUrl: '', minHeightRpx: 0, treatmentClass: '', invalid: false
      })
  ...
  test(`${modulePath}: 默认、规范化、持久化和内容限制`, async (t) => {
    ...
    await t.test('行高可选且按字号倍数保存，旧配置与留白不补行高', () => {
      for (const type of ['EYEBROW', 'TITLE', 'PARAGRAPH', 'LIST', 'HINT']) {
        const original = valid({ type, ...(type === 'LIST' ? { items: ['第一项'] } : {}) })
        assert.equal(Object.hasOwn(rules.normalizeStructuredTextConfig(original).blocks[0], 'lineHeight'), false)
        assert.equal(Object.hasOwn(rules.finalizeStructuredTextConfig(original).blocks[0], 'lineHeight'), false)
        assert.equal(rules.buildStructuredTextPresentation(original).blocks[0].lineHeightStyle, '')
        for (const lineHeight of [0.5, 1.1, 2, 3]) {
          const config = { ...original, blocks: [{ ...original.blocks[0], lineHeight }] }
          assert.equal(rules.validateStructuredTextConfig(config), '')
          assert.equal(rules.finalizeStructuredTextConfig(config).blocks[0].lineHeight, lineHeight)
          assert.equal(rules.buildStructuredTextPresentation(config).blocks[0].lineHeightStyle, `line-height: ${lineHeight};`)
        }
      }
      for (const lineHeight of [0.4, 3.1, 1.15, '1.2', '', null, undefined, NaN, Infinity, '1;display:none']) {
        const config = valid({ lineHeight })
        assert.match(rules.validateStructuredTextConfig(config), /行高|行间距/)
        assert.equal(rules.normalizeStructuredTextConfig(config).blocks[0].lineHeight, lineHeight)
        assert.equal(rules.buildStructuredTextPresentation(config).blocks[0].lineHeightStyle, '')
      }
      const spacer = { blocks: [{ blockKey: 's', type: 'SPACER', lineHeight: 1.2 }] }
      assert.match(rules.validateStructuredTextConfig(spacer, { requireContent: false }), /留白.*行高/)
      assert.equal(Object.hasOwn(rules.finalizeStructuredTextConfig(spacer).blocks[0], 'lineHeight'), false)
      assert.doesNotMatch(rules.buildStructuredTextPresentation(spacer).blocks[0].style, /line-height/)
    })
    ...
    await t.test('校验各独立字段与边界，不接受隐式数字转换', () => {
      for (const patch of [
        { fontSizeRpx: 9 }, { fontSizeRpx: 97 }, { fontSizeRpx: 10.5 }, { fontSizeRpx: '44' },
      ...
      for (const patch of [{ fontSizeRpx: 10 }, { fontSizeRpx: 11 }, { fontSizeRpx: 19 }, { fontSizeRpx: 96 }, { color: '#aAbBcC' }, { marginTopRpx: 128 }]) {
    ...
    await t.test('背景开启必选资源、团队必选成员、拒绝已知无效或音频资源', () => {
      ...
      for (const mediaType of ['IMAGE', 'ANIMATION', 'VIDEO']) {
