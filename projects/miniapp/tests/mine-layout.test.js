...
test('mine action entries use local package image logos and keep content left aligned', () => {
  ...
  assert.match(indexJs, /const FEEDBACK_ENTRY_TYPE = 'feedback'/)
  assert.match(indexJs, /const FEEDBACK_ICON_URL = '\/assets\/system\/wefolio-feedback-icon\.png'/)
  assert.match(indexJs, /const FEEDBACK_PAGE_URL = '\/pages\/feedback\/feedback'/)
  assert.match(indexJs, /type:\s*FEEDBACK_ENTRY_TYPE[\s\S]*title:\s*'问题反馈'[\s\S]*desc:\s*'提交问题、查看处理进度'[\s\S]*iconUrl:\s*FEEDBACK_ICON_URL/)
  assert.match(indexJs, /type === FEEDBACK_ENTRY_TYPE[\s\S]*url:\s*FEEDBACK_PAGE_URL/)
