...
test('video frame probe page is registered and exposes decoder workflow', () => {
  ...
  assert.match(probeJs, /decoder\.start\(\{\s*source:\s*this\.data\.videoPath,\s*abortAudio:\s*true/)
  assert.match(probeJs, /decoder\.seek\(this\.data\.targetTimeMs\)/)
  assert.match(probeJs, /decoder\.getFrameData\(\)/)
