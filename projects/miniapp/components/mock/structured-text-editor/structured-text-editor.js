...
Component({
  properties:{config:{type:Object,value:{}},error:{type:String,value:''},fontAvailability:{type:Object,value:{SYSTEM:true,WECHAT_SANS_SS:false}}},
  data:{draft:{blocks:[]},tab:'content',selectedBlockIndex:0,selectedBlock:null,blockSummaries:[],fontOptions:[],fontLabels:FONT_FAMILIES.map(value=>OPTION_LABELS[value]),weightLabels:FONT_WEIGHTS.map(value=>OPTION_LABELS[value]),alignmentLabels:ALIGNMENTS.map(value=>OPTION_LABELS[value]),blockLabels:BLOCK_TYPES.map(value=>OPTION_LABELS[value]),optionLabels:OPTION_LABELS,optionIndexes:OPTION_INDEXES},
  observers:{config(value){this.refreshDerived(clone(value))},fontAvailability(value){this.refreshFontOptions(value)}},
  methods:{
    refreshFontOptions(value={}){this.setData({fontOptions:FONT_FAMILIES.map(font=>({value:font,label:OPTION_LABELS[font],available:font==='SYSTEM'||value[font]===true}))})},
    refreshDerived(draft,index=this.data.selectedBlockIndex){
      ...
      this.setData({draft,selectedBlockIndex,selectedBlock:blocks[selectedBlockIndex]||null,blockSummaries:blocks.map(block=>({blockKey:block.blockKey,label:OPTION_LABELS[block.type]||block.type,summary:summarizeBlock(block)}))})
    },
    ...
    handleFontTap(event){
      ...
      const draft=clone(this.data.draft);draft.blocks[this.data.selectedBlockIndex].fontFamily=value;this.emitChange(draft)
    },
    ...
    handleChoice(event){
      ...
      draft.blocks[Number(index)][field]=choices[field][Number(event.detail.value)]
      this.emitChange(draft)
    },
