...
const DEFAULT_FONT_OPTIONS = PORTFOLIO_TEXT_SELECTION_OPTIONS.map(item => ({ ...item, available: item.value === 'SYSTEM' }))
const BLOCK_TYPES = [
...
Component({
  properties: {
    ...
    backgroundMembers: { type: Array, value: [] }, backgroundMemberUserId: { type: Number, value: 0 },
    backgroundMembersLoading: { type: Boolean, value: false },
    backgroundResource: { type: Object, value: {} },
    fontOptions: { type: Array, value: DEFAULT_FONT_OPTIONS }
  },
  data: {
  ...
  observers: {
    ...
    fontOptions(options) { this.setData({ fonts: clone(options || DEFAULT_FONT_OPTIONS) }) },
    backgroundResource(resource) { this.applyBackgroundResource(resource) },
    visible(visible) {
  ...
  methods: {
    ...
    applyBackgroundResource(resource = {}) {
      const draft = this.data.draft
      if (!(resource.backgroundLoading || draft.backgroundLoading || draft.backgroundLoadError || draft.backgroundInvalid) || Number(draft.backgroundWorkId) !== Number(resource.backgroundWorkId) || (Number(draft.backgroundMemberUserId) || 0) !== (Number(resource.backgroundMemberUserId) || 0)) return
      this.setData({ draft: Object.assign({}, draft, resource) })
    },
    ...
    beginSession() {
      ...
      this.setData({ draft: normalizeStructuredTextConfig(this.properties.config, { team: true }), blockDraft: null,
        tab: 'content', error: '', revealedKey: '', draggingKey: '', showTypes: false,
        fonts: clone(this.properties.fontOptions || DEFAULT_FONT_OPTIONS) })
      this.refreshRows()
    },
    refreshRows() {
    ...
    handleBlockOption(event) {
      ...
      if (field === 'fontFamily' && !this.data.fonts.some(font => font.value === value && font.available)) { this.setData({ error: this.properties.fontMessage || '字体暂不可用，当前使用系统字体' }); return }
      const blockDraft = clone(this.data.blockDraft)
      if (field === 'fontFamily') {
    ...
    handleSaveBlock() {
      ...
      let error = validateStructuredTextConfig(validation, { team: true })
      if (block.type === 'SPACER' && error === '请至少添加一个非空文字或列表区块') error = ''
      if (error) { this.setData({ error }); return }
      draft.blocks = finalizeStructuredTextConfig(validation, { team: true }).blocks
    ...
    handleConfirm() {
      if (this.data.blockDraft || this.data.tabSwitching) return
      if (this.data.draft.backgroundEnabled && (this.data.draft.backgroundLoading || this.data.draft.backgroundLoadError)) {
        this.setData({ error: this.data.draft.backgroundLoadError || '背景作品加载中，请稍候' }); return
      }
      const error = validateStructuredTextConfig(this.data.draft, { team: true })
      if (error) { this.setData({ error }); return }
      this.triggerEvent('confirm', finalizeStructuredTextConfig(this.data.draft, { team: true }))
    }
  }
})
