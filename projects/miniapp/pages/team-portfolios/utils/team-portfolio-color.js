...
function normalizeTeamHexColor(value) {
  ...
  return TEAM_HEX_COLOR_PATTERN.test(color)
    ? color.toUpperCase()
    : DEFAULT_TEAM_BACKGROUND_COLOR
}

function teamHexToRgb(value) {
  const color = normalizeTeamHexColor(value)
  return {
...
function teamRgbToHex(red, green, blue) {
...
function hexToHsv(value) {
  const { red, green, blue } = teamHexToRgb(value)
...
function hsvToHex(hsv = {}) {
  ...
  return teamRgbToHex(...channels.map((channel) => (channel + match) * 255))
}
