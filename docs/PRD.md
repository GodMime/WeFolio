...
本文档以 `design/prototype.html` v1.2 的 27 个页面设计稿为主要交互依据，并保留设计稿尚未覆盖但属于 MVP 闭环的业务能力。页面文案用于说明业务意图，最终上线文案可在不改变业务规则的前提下优化。
...
| v1.1 | 2026-06-22 | 对齐详细设计稿；补充工作台指标、作品批量管理、成员展示权限、作品集多实例与发布规则、AI 失败扣费规则、访客档期筛选及页面覆盖矩阵 |
| v1.2 | 2026-06-22 | 团队角色调整为拥有者、管理者、普通成员；三种角色均可查看成员，明确成员维护、团队作品集管理、预览和分享的角色边界 |
...
- 同一维护者可以同时加入多个团队；其在不同团队中的角色、加入状态和展示权限相互独立。
...
- 成员展示权限：团队拥有者为成员设置的公开范围，分为公开、仅团队可见、隐藏，并可分别控制个人作品集、头像资料和个人作品素材能否被团队作品集引用。
...
- 团队创建、团队唯一码生成、拥有者/管理者/普通成员三级角色、通过个人唯一码邀请成员、成员确认及展示权限管理
- 标准团队作品集创建、预览、发布、分享
...
- 绑定成功后生成不可重复的个人唯一码。
- 个人唯一码可复制、可展示为二维码。
- 注册、授权登录或手机号绑定成功后进入“我的”页；未完成手机号绑定的账号先进入手机号绑定流程。
...
- 只有团队拥有者可以添加、移除成员，设置管理者/普通成员角色，以及配置公开、仅团队可见、隐藏三种展示范围和内容引用权限。
...
1. 我的：原工作台页面，展示个人信息、积分余额，并承载访问记录和“我的团队”入口。
...
访问记录不再作为底部主导航项；“我的团队”不进入底部主导航，统一从“我的”页进入，并根据用户在各团队中的角色展示可用功能。
...
- 使用维护者能理解的业务语言解释组件，例如“轮播图用于首屏展示代表作品”“档期查询用于让客户查看可约时间”“扫码联系用于承接咨询”。
...
- 维护页：仅拥有者和管理者可进入，用于配置团队作品集素材、成员作品集入口、团队档期组件、保存草稿、预览和发布。
...
- 上传一个作品消耗 1 积分；批量上传按成功保存的作品数量累计扣除。
...
- 上传作品在上传成功并保存作品记录后扣除积分；批量上传按成功保存的作品记录数扣除。
...
页面清单按设计稿逻辑分组：登录 → 我的/积分/跟进/团队管理 → 作品 → 作品集维护与预览 → 访客展示 → 档期维护。
...
| 03 | 我的积分 | 维护者 | 余额、累计充值/消耗、分类指标、积分流水 |
| 04 | 充值积分 | 维护者 | 充值档位选择、支付金额和到账积分确认 |
| 05 | 积分规则 | 维护者 | 维护消耗、访客消耗、账户归属和维护中规则 |
| 06 | 访问记录 | 维护者 | 访问指标、趋势、来源与行为摘要、跟进状态 |
| 07 | 团队成员列表 | 三种团队角色 | 三种角色均可查看和搜索已加入成员；拥有者额外显示待确认等状态筛选和成员维护操作，其他角色为只读模式 |
| 08 | 团队成员维护 | 团队拥有者 | 唯一码匹配、管理者/普通成员角色、职业、展示范围、引用权限和邀请确认 |
| 09 | 作品 | 维护者 | 搜索、标签筛选、引用次数和批量管理 |
| 10 | 批量添加作品 | 维护者 | 最多 9 个素材、文件列表和统一标签 |
| 11 | 作品编辑 | 维护者 | 标题、标签、说明及引用明细 |
| 12 | 作品集 | 维护者 | 个人/团队筛选、角色标识及按拥有者/管理者/普通成员展示管理、预览、分享操作 |
| 13 | 标准个人作品集维护 | 维护者 | 分享信息、固定组件、草稿、预览和发布 |
| 14 | 标准个人作品集预览 | 维护者 | 模拟访客效果，保留返回且不统计、不扣分 |
| 15 | 组件库简介 | 维护者 | 用业务语言说明高级作品集可用模块 |
| 16 | 高级个人作品集维护 | 维护者 | 素材编号、自然语言、AI 生成、组件调整和发布 |
| 17 | 高级个人作品集预览 | 维护者 | 高级视觉预览，保留返回且不统计、不扣分 |
| 18 | 访客个人作品集 | 访客 | 标准个人作品集浏览、档期查询和扫码联系 |
| 19 | 访客高级作品集 | 访客 | 高级个人作品集浏览、档期查询和扫码联系 |
| 20 | 团队展示与成员 | 访客 | 团队介绍、聚合档期、成员作品集和团队联系 |
| 21 | 访客档期查询 | 访客 | 日期、查询范围、档位状态和结果提示 |
| 22 | Under Maintenance | 访客 | 余额不足时保留页面骨架并遮罩内容区 |
| 23 | 档期 | 维护者 | 月历概览、日期明细和当天维护入口 |
| 24 | 档位定义 | 维护者 | 全部定义、启用/停用和新增入口 |
| 25 | 编辑档位定义弹层 | 维护者 | 名称、状态、起止时间、颜色和快照提示 |
| 26 | 新增档位定义弹层 | 维护者 | 新建档位定义但不自动创建具体日期档期 |
| 27 | 档期维护 | 维护者 | 选择档位、状态、联系人、电话、备注、删除和保存 |

### 8.2 设计稿尚未覆盖但 MVP 必须补齐的页面或状态

- 手机号补充绑定、短信验证码异常和账号密码找回。
- 个人资料编辑、个人唯一码复制/二维码展示。
- 团队列表、团队创建、团队资料编辑、邀请确认和团队所有权转移。
- 标准/高级团队作品集维护页与预览页；当前设计稿仅覆盖团队访客展示页。
- 连续日期批量维护档期；当前设计稿只覆盖单日单条档期编辑。
...
| displayScope | enum | public / team_only / hidden |
...
| eventType | enum | open / view_work / play_video / query_schedule / qr_action / open_member_portfolio |
...
9. 维护者保存草稿并进入预览页确认效果；预览页内任何查看、播放、档期查询、二维码点击不扣除积分。
...
2. 输入成员个人唯一码。
3. 系统查询维护者。
4. 拥有者选择管理者或普通成员角色，并设置职业身份、展示范围和可引用内容。
...
- 团队作品集只能引用已加入成员明确授权的内容；展示范围为隐藏的成员不得出现在访客端。
...
- 对齐 27 个页面设计稿，补齐作品批量管理、访问趋势、团队成员确认与展示权限、作品集草稿/发布版本、AI 失败不扣分及访客档期筛选。

### v1.2 团队权限细化（当前版本）

- 团队角色调整为拥有者、管理者、普通成员，并明确成员维护、团队作品集管理、预览和分享权限。

### v1.3

- 增加会员套餐、作品集模板市场、更多 AI 文案能力和更完整的团队协作权限。
...
- 连续日期批量维护档期采用独立页面、底部弹层还是复用单日维护页？
- 团队成员收到邀请、接受、拒绝和权限变更时采用小程序内消息还是微信订阅消息？
