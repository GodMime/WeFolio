...
- 支持图片、视频上传。
- 支持标题、类型、标签、说明、排序、上下架状态。
- 支持删除与替换素材。
- 已被作品集引用的作品删除时需提示影响范围。

### 5.3 维护个人档期

作为维护者，我希望按日期自定义当天有哪些档位，并为每个档位设置起止时间，便于客户按具体时间段查询。

验收标准：

- 支持月历概览和按日期列表查看每天已维护的档位、时间和状态。
- 系统默认提供中午档和晚间档。
- 默认中午档时间为 10:00-13:00，默认晚间档时间为 17:00-20:00。
- 支持每天自定义新增、编辑、删除档位，例如迎亲档、午后仪式档、After Party 档。
- 支持为每个档位维护展示颜色，默认提供 8 种颜色可选，月历当天格子按档位颜色进行多色彩标记。
- 支持按时间段标记空闲、已约、待定、休息。
- 支持批量设置日期或连续日期的多个时间段状态。
- 支持给某天或某个时间段添加备注，仅维护者可见。
- 访客只能看到是否可预约，不展示私人备注。
...
1. 我的：原工作台页面，展示个人信息、积分余额、今日待办，并承载访问记录和团队管理入口。
2. 作品：原作品列表页，作为一级 tab 页面，不展示返回按钮。
3. 作品集：原作品集列表页，作为一级 tab 页面，不展示返回按钮。
4. 档期：原档期维护页，作为一级 tab 页面，不展示返回按钮。
...
- 维护者可以按天新增多个档位。
- 每个档位需要包含档位名称、开始时间、结束时间、状态和展示颜色。
- 档位颜色选择器默认提供 8 种颜色；颜色用于月历格子的多色彩标记，同一天存在多个档位时，月历当天显示多个对应颜色。
- 自定义档位只影响指定日期；后续可扩展为保存成个人常用档位模板。
- 同一天内档位时间不建议重叠，如存在重叠需提示维护者确认。
...
- 作品新增/编辑页
- 档期管理页（一级 tab：档期，无返回按钮，月历色彩标记 + 每天档期列表）
...
| date | date | 日期 |
| slotId | string | 档位 ID |
| slotName | string | 档位名称，例如中午档、晚间档、迎亲档 |
| startTime | string | 时间段开始时间 |
| endTime | string | 时间段结束时间 |
| slotColor | string | 档位展示颜色，用于月历多色彩标记 |
