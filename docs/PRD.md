...
| v2.1 | 2026-08-13 | 新增个人作品集访客查档与新版留资扣费规则，明确独立两小时窗口、自访不落业务数据及负余额记账语义 |
| v2.2 | 2026-09-03 | 统一高级作品集计费口径：AI 生成、重新生成、保存和预览不扣积分，仅发布成功时扣 20 积分 |
| v2.3 | 2026-09-03 | 明确模型 JSON 校验失败后的同任务自动纠正重试 |
| v2.4 | 2026-09-03 | 补齐高级视觉参数的中文名称、用途说明和枚举中文标签要求 |
| v2.5 | 2026-09-03 | 高级作品集第 3 步复用普通作品集编辑能力，并增加可编辑的扩展参数控件 |
| v2.6 | 2026-09-03 | 去除“页面主题”，固定页面与各组件高级参数的枚举、数值范围、步进和适用关系 |
| v2.7 | 2026-09-04 | 统一作品集为单一标题，取消发布前强制预览和联系方式组件必选要求，并限制每个用户同时只能有一个活动 AI 任务 |
| v2.8 | 2026-09-05 | 明确普通与高级作品集共用编辑组件且普通页面不显示高级参数；积分扣除数值统一以当前数据库生效规则为准 |
| v2.9 | 2026-09-05 | 更正高级第 3 步页面边界：保留带步骤条的独立高级编辑页面壳，仅复用普通作品集编辑组件 |
| v2.10 | 2026-09-08 | 高级新建必须经 AI 生成；推荐封面复用普通上传流程，补齐重开再生成、文字组件衔接及高级访问记录和留资要求 |
...
- 团队作品集：面向访客展示的团队展示页，分为标准团队作品集和高级团队作品集；团队不设积分账户，团队作品集仅在发布成功时由实际发布人承担对应积分。
...
- 新增作品时，作品标题优先使用选择器提供的原始文件名（去扩展名，最多 30 字）；只有临时路径时，使用“图片/视频/音频 + 选择时的本地日期时间（精确到秒）+ 两位序号”，例如“视频 2026-09-16 17:06:32 01”，保存后可修改。
- 作品说明可先为空，保存后再补充。
...
音频作品补充（2026-09-10，本地实现，待真机与云服务验收）：

- 新增“上传音频”入口，支持 MP3/M4A/AAC/WAV，单个 ≤50MiB、时长 >0 且 ≤10 分钟，单批最多 9 个，每用户最多 100 个；不限制码率。前端用微信原生能力读取时长并在申请上传票据前校验，沿用现有 COS 直传流程，后端不接收或解析音频文件。
- 新增时使用默认封面 `system/default-audio-cover-v1-200kb.png`；后续只能选本人可用、审核通过的图片作品原件作为封面，不复制文件。删除图片时按对象 key 检查音频封面引用；删除音频不删除共用封面。
- 作品库支持音频试听及既有标题、标签、说明、排序、删除和内容审核；存储积分 job 按文件大小统计音频，音频不新增固定上传扣费，也不发送视频播放事件。
- 个人和团队作品集均支持全局背景音频，默认关闭、可以不选，通过“从音频作品中选择”引用。关闭保留选择与引用，移除才清空；团队候选沿用成员作品授权。
- 设置区提供轻量唱片、封套抽盘、迷你播放器三款真实样例，统一使用所选音频封面；编辑器仅手动试听。预览/访客开启且音频可用时右上角显示控件，首次尝试自动播放一次；失败、切后台或视频结束后不自动恢复。播放视频暂停背景音频，手动播放音频先停止当前视频，退出页面销毁播放器。
- 旧版本无音频入口、无历史音频作品/背景配置；后端先发布，小程序审核约延后一天，期间不会产生音频数据，发布后依靠自动升级。不增加客户端能力头或按版本隐藏音频；缺少背景配置字段的保存请求保留已有配置及引用。
...
- 第 3 步使用带步骤条的独立高级个人作品集维护页；步骤条下挂载与标准个人作品集共用的编辑组件，复用分享信息、页面设置、组件编排及其交互能力。
- 高级页面让共享编辑组件显示扩展参数控件，并提供返回重新生成入口；普通作品集仍进入原标准编辑页，该页面不显示步骤条，也不让共享组件渲染任何高级参数控件。
- 高级作品集包含维护页、预览页、访客页。
- 维护页支持保存草稿、重新生成、预览和发布。
- 预览为可选操作，不作为发布前置条件；预览页和访客页展示效果一致，预览页有返回按钮，预览页内任何查看、播放、档期查询、二维码点击均不扣除积分。
...
- AI 输出结构化组件方案，系统渲染预览。
- AI 同时生成作品集标题并写入页面配置，维护者可在现有标题控件中修改。
- 模型输出未通过 JSON 或 schema 校验时，后端在同一生成任务内自动纠正重试，耗尽后再提示失败。
- 拥有者或管理者确认后发布。
...
- 可查看微信二维码并长按识别。
- 访问行为会形成访问记录。
- 访客成功查询个人作品集档期时扣除作品集所属维护者 10 积分；同一全局访客查询同一作品集距上次成功扣费不足 2 小时时不重复扣分。
- 维护者自访查询自己的作品集时仅返回实时档期结果，不写入查询事件、查询快照或统计，也不扣除积分。
...
- 仅通过新版访客留资接口成功创建新线索时扣除作品集所属维护者 10 积分；同一全局访客向同一作品集留资距上次成功扣费不足 2 小时时不重复扣分，幂等命中既有线索不扣分。
- 查档与留资分别使用独立扣费窗口，两种行为在同一 2 小时内各自最多扣除 10 积分，互不抵扣。
- 普通访客对同一作品集终身最多创建 3 条线索；若相邻线索均间隔不少于 2 小时，最多触发 3 次留资扣费。
- 维护者不能在自己的作品集预留联系信息；请求按“访问记录无效”拒绝，不创建线索、不写入行为事件，也不扣除积分。
...
- 个人简介
- 微信二维码
- 联系手机：默认显示登录手机号，允许修改或明确清空，清空后不再自动回填
- 联系微信：独立文本字段，允许修改或清空
...
联系手机、联系微信分别最多 32、64 个 Unicode 字符，去除首尾空格，不允许换行或控制字符。资料接口省略字段或传 null 表示不修改，空串表示明确清空；联系手机未设置时由服务端取登录手机号。两项独立于微信二维码维护。
...
- 批量上传保存时，每个文件生成一条作品记录；作品标题优先使用原始文件名，拿不到原名时使用“图片/视频/音频 + YYYY-MM-DD HH:mm:ss + 两位序号”，不得将临时路径文件名作为标题。日期时间用于区分同一天不同时间选择的上传批次；序号在当前上传页面内按已接纳文件递增，分批选择、删除或编辑标题后继续追加不会重用编号；后续可在作品编辑页修改标题。
...
- 同一维护者所有未删除档位定义的展示颜色必须唯一；停用档位仍占用颜色，逻辑删除后颜色可以重新使用。新增和编辑均需校验，编辑时排除当前档位自身。
...
- 微信二维码
- 发布状态：草稿 / 已发布
- 分享封面、分享头像和标题共同用于分享卡片与访客首屏；缺少必填分享信息时不得发布。
...
#### 7.5.1 本次个人、团队标准作品集组件扩展

- 个人与团队的文字说明、结构化文字说明和文字网格字号范围统一为 10–96 rpx 整数。文字说明保留 26、28、32、36 四个快捷档位，已有范围内的其他字号继续显示为自定义选项；结构化文字说明和文字网格支持整数输入，加减按钮步长统一为 1 rpx。各组件原有默认字号保持不变，编辑、保存、预览与访客展示使用一致的范围。
- 三类文字组件支持可选行间距 `lineHeight`，按字号倍数调节，范围 0.5–3.0，步长 0.1，可输入或通过加减按钮调整。文字说明按整个正文设置，结构化文字说明按非留白区块设置，文字网格按段落设置并作用于该段所有文字片段。未设置时保留原有行距，打开编辑或未调整就保存不补写默认值；非法输入阻止保存，取消丢弃修改。个人、团队的编辑、预览和访客展示同步生效，网格调整后重新测量内容高度；旧客户端保存时省略该字段不会清除已配置的行距。
- 标准个人作品集在“页面设置”的“背景色”之前提供“组件间距”，范围为 0–96 rpx 整数，默认 32 rpx；统一控制各菜单内组件外层的上方间隔，组件自身的外侧留白继续叠加。保存后预览和已发布访客页按对应版本展示；旧作品集缺省使用 32 rpx，旧版编辑器保存时保留已配置的间距。
- 个人资料组件可切换纵向／横向布局，默认纵向保持现有外观；横向采用左侧圆形头像、右侧姓名／职业／城市，简介、标签与二维码在下方完整展示，继续遵循各字段的显示开关。刷新基础资料只替换资料内容，保留已选布局；个人预览和访客页同步生效。
- 个人资料卡片支持边框开关，默认关闭；开启后可滑动调整线框粗细，范围 1–12 rpx 整数，默认 1 rpx。边框颜色默认跟随主题，可选黑色、白色、浅灰或自定义 HEX，复用现有内联颜色选择器。关闭边框保留已选粗细和颜色，刷新基础资料保留布局及边框设置；纵向、横向布局以及预览、访客页均同步生效，旧版编辑器保存时保留已配置的边框。
- 个人资料卡片开启边框时，可分别滑动设置左右、上下外侧留白，范围均为 0–96 rpx 整数，默认左右 32 rpx、上下 0；留白位于整张卡片边框外侧，上下距离叠加在既有组件间距上，边框内侧保留原有 32 rpx 内边距。关闭边框时不应用外侧留白，但保留设置；取消编辑丢弃未确认调整，刷新资料不覆盖留白，保存后预览和访客页同步展示，旧版编辑器保存时保留新版留白配置。
- 文字网格：从空白 2×2 开始，直接设置 1–8 行、1–5 列，再选择完整矩形合并或拆分；不提供预设布局入口。支持 5×4 合并首行得到“一个通栏＋4×4”共 17 格，以及 2×2 合并首列得到“左侧大格＋右侧1×2”共 3 格。减少行列导致文字越界时提示移走或清空并阻止调整，合并按阅读顺序保留段落，拆分内容留在左上格，支持撤销。
- 网格每格最多 8 段、每段最多 8 片段、整个组件合计最多 2000 Unicode 字符。每段设置对齐和段距，每片段单独编辑内容、字号、常规/加粗、系统/微信字体与颜色；颜色沿用“跟随系统、白色、黑色、自定义”及现有自定义颜色弹层。默认最小行高 180rpx，按实际换行增高；测量失败时自然纵向排布以保证文字可读。
- 文字网格外侧留白可分别设置左右、上下距离，均为 0–96 rpx 整数，默认 0；左右留白从整体可用宽度扣除，按剩余宽度计算各列和文字换行，上下留白位于整个网格外侧。设置过大导致文字可用宽度不足时阻止调整并回显原值，支持撤销，保存后个人、团队预览与访客页一致展示。
- 网格外观“细边框”默认关闭，开启后在下方展开线框宽度与颜色；线宽可输入或滑动调整为 1–12 rpx 整数，默认 1 rpx；颜色提供黑色 `#000000`、白色 `#FFFFFF`、灰色 `#D7DADD` 及自定义，新建默认灰色。自定义在同一弹层下方展开色板、色相、HEX 与确认/取消，不打开新弹层；取消丢弃临时颜色并收起。关闭细边框时隐藏宽度和颜色配置、不绘制边框，保留已选值供下次开启；个人、团队的预览及访客页同步生效。
- 联系信息组件：新增时读取当前编辑者的联系手机和微信，团队也取当前编辑者资料；保存后固定为组件快照。组件内允许分别修改，“重新填入”只更新面板副本，取消丢弃。预览、访客页直接展示非空项并逐项提供复制，资料后续修改不自动覆盖已存快照。
- 联系信息支持边框开关，默认关闭；开启后可调整上下、左右外侧留白（0–96 rpx，默认 0）、线宽（1–12 rpx，默认 1）和边框颜色（默认跟随主题，支持黑、白、浅灰及自定义 HEX）。尺寸只使用数值步进控件，支持直接输入及按 1 rpx 加减；关闭不应用边框及外侧留白，但保留配置，再次开启继续使用。重新填入资料保留边框设置，取消丢弃本次修改，个人、团队的保存、预览及访客页一致生效；旧版编辑器保存时保留新版边框字段。
- 视频轮播：保留默认叠放样式，增加竖向卡片横滑样式，卡片约占轨道宽度 80%、封面比例 4:5，支持吸附与位置指示；两种样式均支持作品标题及描述，新增描述开关默认关闭，切换样式保留作品顺序与文案配置。
- 视频轮播组件标题：提供独立显示开关，默认开启，旧配置缺省时继续显示原标题；关闭后隐藏标题及占位，保留文字供再次开启，不影响各作品标题开关。旧客户端保存时保留已有开关状态。
- 视频轮播编辑：个人与团队均采用紧凑横排样式选择、标题输入及开关，作品说明按需查看；展示设置默认展开并支持收起，搜索、筛选和作品顺序保持独立，作品列表使用紧凑行与轻分隔线。
- 单个作品：新增“原位展示／独立详情页”打开方式，默认原位展示。详情标题、描述分别配置且默认开启；个人、团队均从来源页已授权的作品快照进入，返回保留菜单和位置。详情不支持分享直达，冷启动或缺失来源上下文时显示不可用；详情需要刷新授权时返回来源页重新打开；预览进入详情不产生访客统计。
- 以上扩展不接入 mock 体验版，mock 保持独立组件能力与本地数据。
...
AI 生成阶段响应示例（素材编号由 Job 转换后才形成最终配置）：

```json
{
  "schemaVersion": "advanced-ai-candidate-v1",
  "shareCoverSourceNo": 1,
  "config": {
    "share": { "title": "温柔纪实婚礼作品集" },
    "components": [
      {
        "componentKey": "c_carousel_1",
        "componentType": "CAROUSEL",
        "enabled": true,
        "sortOrder": 1000,
        "config": { "sourceNos": [1, 2, 3] }
      }
    ]
  }
}
...
- 标准与高级作品集都只有一个标题，统一存放在配置 JSON 的 `share.title`，用于维护列表、分享卡片和访客页面，不新增配置外标题字段。
- 高级新建必须先由 AI 生成成功，再进入第 3 步编辑；不提供跳过 AI 或空白手工创建入口。AI 关闭、失败或超时保留来源与描述并允许后续重试，不能从产品步骤条绕过生成。
- AI 生成标题后允许维护者通过现有标题控件手工修改。首次完整保存必须关联成功生成任务；已经保存的高级草稿可直接重开、手工编辑、保存、预览和发布，不依赖 AI 开关。
- AI 可推荐本次所选图片作为封面；推荐图先下载为本地临时图片，经维护者按普通流程确认或裁剪，再上传为当前作品集自有素材。失败可重试、主动移除推荐或手工选图；不能直接把成员作品 URL 写成团队草稿封面。
- 从统一作品集列表按作品集 ID 重开时读取现有草稿，不依赖本地生成会话。重开后重新生成回到第一步重选来源、填写新描述，并绑定原作品集；不恢复历史来源或任务，新结果再次保存前已保存草稿不变。
- 高级手工编辑承接结构化文字和文字背景扩展，个人 13 类、团队 12 类组件及现有文字样式可保存、预览和发布；AI 首版生成子集单独定义，结构化文字与文字背景由维护者在生成后添加，不因 AI 白名单而拒绝合法手工配置。
- 高级视觉参数必须提供中文名称和用途说明，枚举取值同时提供中文标签；高级模式下通过中文控件手工配置，不直接显示英文参数键名。
- 首版页面高级参数只包含内容左右留白、组件间距和统一圆角，范围均为 0～64rpx、步进 4rpx；不提供页面主题参数。
- 组件高级参数严格使用高级作品集方案定义的完整白名单；枚举不得增加未定义值，数值不得超出方案规定的上下界或步进，不适用于当前组件的参数必须拒绝。
- 高级作品集第 3 步使用独立高级维护页面壳，顶部保留步骤条和返回重新生成入口；步骤条下挂载个人或团队共享编辑组件，按“分享信息 → 页面设置 → 组件编排”复用字段、编辑交互和保存能力，并启用扩展参数控件。
- 普通作品集仍进入原 `standard-edit` 页面，并挂载同一归属类型的共享编辑组件；普通页面不渲染步骤条，不启用任何高级参数控件，也不在保存请求中携带高级配置字段。普通与高级分别使用独立页面路由和页面壳。
- AI 只能生成白名单组件和配置，不生成可执行代码。
- 系统校验模型引用的 `sourceNo` 是否存在。
- `sourceNo` 只用于生成请求、任务快照和模型响应；Job 在生成成功前将编号转换为标准组件已有的真实引用字段。最终作品集配置不保存来源编号或重复的来源清单，保存时继续按组件真实引用重建 `wf_portfolio_reference`。
- 预留联系信息组件仅允许配置字段显隐、必填规则和说明文案，不允许改变线索接收方；联系人必填，手机和微信至少保留并要求填写一项。
- 联系方式组件为可选组件，作品集不包含预留联系信息或微信二维码时仍可保存和发布。
- 同一用户同时最多只能有一个待执行或执行中的 AI 生成任务；相同幂等请求返回原任务，其它请求提示已有任务进行中。
- AI 生成、重新生成和保存草稿不扣积分；发布前展示预计消耗 20 积分，余额不足时禁止发布并提供充值入口。
- 模型 JSON 语法、schema、素材序号或参数校验失败时，后端在同一任务内最多自动纠正 2 轮；仍失败或生成超时时保留已选择素材和自然语言描述，支持用户直接重新生成。
...
- 维护者可选择预览后再发布，也可在保存草稿后直接发布；预览不改变发布资格。预览页展示效果与访客页一致，但顶部保留返回维护页按钮。
- 预览页内任何查看、播放、档期查询、二维码点击不扣除积分，也不计入访客访问统计。
- 高级访客页支持真实访问记录、媒体与查档事件、联系线索提交及维护端查看，沿用既有访客身份、发布版本、权限、幂等和生效计费规则；不能仅实现高级渲染而遗漏业务记录。
...
1. 选择素材：团队有权使用的成员已审核作品、成员已发布标准或高级个人作品集。
2. 系统给已选对象分配序号。
3. 输入自然语言布局描述。
4. AI 生成包含作品集标题的结构化页面 schema。
5. 系统渲染预览。
...
- 维护页：仅拥有者和管理者可进入；前两步用于选择素材和自然语言生成，第 3 步使用带步骤条的独立高级团队页面壳，下方挂载团队共享编辑组件以复用标题、分享信息、页面设置、组件编排和保存能力，并启用扩展参数控件。
...
- 高级团队的新建、推荐封面处理、已有草稿重开和重新生成遵循高级个人相同规则，不提供跳过 AI。正式入口位于统一作品集列表，历史团队列表页仅跳转。
- 高级团队开页写访问记录，媒体与查档事件、访客留资和维护端线索查看完整可用；团队访问仍不扣积分。删除被引用的成员作品集、撤销授权和移除成员时，高级团队父作品集与标准团队使用同一引用保护。
- 当前暂无正式用户的团队，按本次用户决定不将旧版团队客户端写回高级配置作为本阶段兼容验收；其它标准接口和行为仍保持兼容。
...
- 每次成功提交需关联作品集、发布版本、访问记录和来源；线索提交作为访问行为事件记录。仅新版访客留资接口成功创建新线索时按访客留资积分规则扣费，旧版接口不扣费。
...
- 保留原访问记录页面，只在事件明细弹层增加“累计前台停留”和“最近采集设备”两行，设备可原位展开品牌、型号、系统、平台和采集时间；切换记录收起详情。
- 停留仅累计成功展示作品集首页、菜单及本次新增详情页的有效前台时间；后台、其他页面、预览、维护中和失败状态不计，合法成功展示的空作品集计入。个人维护者本人自访不计，团队拥有者从正式访客入口自访计入。维护者登录或令牌失效不清理独立的访客补报队列。
- 新增独立的毫秒累计字段。历史未采集为 null，显示“暂无停留数据”；首次有效 0 值显示“不到 1 秒”。旧事件及汇总秒数字段保持原行为，不回填或混用。设备与停留独立，允许设备已知而停留尚未采集。
- 同次浏览及 401 刷新复用同一打开键和活动键，恢复不新增打开事件或再次触发打开扣费，跨越两小时窗口也相同；真实重新进入新建双键并继续按原计费窗口判断。每 15 秒上报会话累计高水位，退出尽力补报；本地队列最多 50 条、7 天，不保存令牌。异常终止或清理缓存可能丢失未确认时间。
- 设备从最近一次含有效设备信息的活动会话读取，按会话创建时间及 ID 倒序取值；旧会话延迟补报不改变“最近设备”。无数据展示“设备未知”；部分缺失时摘要只展示已知项，展开明细保留品牌、型号、系统和平台四行，缺失值显示“未知”。
- 设备采集须在微信公众平台《用户隐私保护指引》补齐品牌、型号、系统和平台声明并完成配置核验后启用；用途为向作品集维护者展示本次访问设备，保存期限与访问记录规则一致。平台核验记录缺失时保持设备采集关闭，停留计时独立工作。

### 7.12 积分与充值

积分扣除数值以 Runtime 最新 Flyway migration 中当前生效的 `wf_point_rule` 为唯一口径；PRD 其它章节或历史迭代摘要与本节冲突时，均以本节及数据库生效规则为准。

积分用途：

- 上传一个图片或动图消耗 5 积分，上传一个视频消耗 10 积分；批量上传按成功保存的作品类型和数量累计扣除。
- 新建团队消耗 2000 积分，由创建团队的维护者个人账户扣除。
- 成功发布一次标准作品集消耗 10 积分，个人作品集扣所属维护者，团队作品集扣本次执行发布的维护者。
- 成功发布一次高级作品集消耗 20 积分，个人作品集扣所属维护者，团队作品集扣本次执行发布的维护者。
- 从 2026-07 账期开始，作品存储月费按每完整 2MB 扣 1 积分，不足一个完整计费单位不扣分。
- 访客访问一次个人标准作品集消耗 10 积分，同一访客访问同一作品集距上次成功扣费不足 2 小时时不重复扣分。
- 访客访问一次个人高级作品集消耗 10 积分，同一访客访问同一作品集距上次成功扣费不足 2 小时时不重复扣分。
- 团队无积分账户，团队作品集访问无消耗；访客从团队作品集跳转到个人作品集后，再按个人作品集规则扣除所属维护者积分。
- 访客在个人作品集每次查看图片大图消耗 1 积分，同一访客查看同一作品距上次成功扣费不足 2 小时时不重复扣分。
- 访客在个人作品集每次查看视频消耗 10 积分，同一访客查看同一作品距上次成功扣费不足 2 小时时不重复扣分。
- 访客成功查询个人作品集档期消耗 10 积分，同一全局访客查询同一作品集距上次成功扣费不足 2 小时时不重复扣分。
- 访客通过新版留资接口成功创建一条个人作品集线索消耗 10 积分，同一全局访客向同一作品集留资距上次成功扣费不足 2 小时时不重复扣分。
- 维护者预览页内的查看、播放、档期查询、二维码点击不扣除积分。
- AI 生成、重新生成、保存草稿、预览以及生成失败、超时或输出校验失败均不扣除高级作品集发布积分。
...
- 标准作品集仅在成功发布后扣 10 积分；保存草稿、打开编辑页、修改未保存、预览或失败请求不扣分。
- 高级作品集仅在发布成功后扣 20 积分；AI 生成、重新生成、保存草稿、打开编辑页、修改未保存、预览或失败请求不扣分。
- 团队不设积分账户；团队作品集发布积分由实际发布人承担，访客访问团队作品集本身不扣积分；访客从团队作品集跳转后，消耗个人作品集所属维护者的个人作品集积分。
- 上传作品在上传成功并保存作品记录后扣除积分；图片和动图扣 5 积分，视频扣 10 积分；批量上传按成功保存的作品类型和数量累计扣除。
- 从 2026-07 账期开始，作品存储月费按总文件大小中的完整 2MB 单位计费，每个完整单位扣 1 积分，未满 2MB 的剩余部分不进位。
- 新建团队在团队创建成功并生成团队唯一码后扣除 2000 积分。
- 标准个人作品集在发布成功后，从作品集所属维护者账户扣除 10 积分；高级个人作品集在发布成功后，从作品集所属维护者账户扣除 20 积分。
- 标准团队作品集在发布成功后，从本次执行发布的维护者账户扣除 10 积分；高级团队作品集在发布成功后，从本次执行发布的维护者账户扣除 20 积分。
- 访客访问个人作品集在页面成功打开时扣除 10 积分；后端按被扣费维护者 + 积分场景 + 全局访客 ID + 作品集 ID 维护滚动窗口，距上次成功扣费不足 120 分钟时不重复扣分。
- 访客访问团队作品集不扣除积分；从团队作品集跳转到个人作品集后，在个人作品集页面成功打开时扣除个人作品集所属维护者积分，同样遵循滚动 2 小时规则。
- 图片大图每次查看扣除 1 积分，后端按被扣费维护者 + 图片查看场景 + 全局访客 ID + 作品 ID 维护滚动窗口，距上次成功扣费不足 120 分钟时不重复扣分。
- 视频每次播放或打开详情扣除 10 积分，后端按被扣费维护者 + 视频查看场景 + 全局访客 ID + 作品 ID 维护滚动窗口，距上次成功扣费不足 120 分钟时不重复扣分。
- 个人作品集档期查询成功后扣除 10 积分；新版访客留资接口成功创建新线索后扣除 10 积分。两者分别按被扣费维护者 + 积分场景 + 全局访客 ID + 作品集 ID 维护独立滚动窗口，距各自上次成功扣费不足 120 分钟时不重复扣分。
- 朋友圈匿名访客按同一作品集的聚合身份共享全局访客 ID，因此同一作品集 2 小时内全部朋友圈匿名访客合计最多触发一次查档扣费和一次留资扣费。
- 维护者自访自己的作品集时，查档仅返回实时结果且不记录任何业务数据；留资按访问记录无效拒绝。两种自访行为均不扣分。
- 同一作品出现在同一维护者的多个个人作品集中时共享媒体查看窗口；不同访客、不同作品和不同积分场景分别计费。
- 每次扣费必须生成积分流水，包含业务类型、业务 ID、扣费积分、扣费前余额、扣费后余额。
- 访客已经进入可访问页面后触发的访问、媒体查看、查档或新版留资扣费不因本次扣费导致余额不足而失败或回滚；允许余额变为负数，并完整写入积分流水、低余额消息与维护任务，访客下次刷新作品集时进入维护中状态。
...
- 同一业务动作的重复请求不得重复扣分；真实计费故障（例如规则缺失、窗口锁、账户或流水写入异常）时，业务动作与积分变更必须在同一事务内回滚并返回全局异常，不得返回成功结果。
...
| title | string | 作品标题，新增时优先使用原始文件名；仅有临时路径时使用类型、本地日期时间和序号 |
...
2. 系统展示素材序号。
3. 维护者输入自然语言描述。
4. 后端在用户粒度分布式锁内检查活动任务并创建 AI 任务；同一用户同时最多存在一个 `PENDING` 或 `RUNNING` 任务，生成、重新生成和失败均不扣积分。
5. 后端调用 AI 生成包含 `share.title` 的完整 schema，并校验标题、组件白名单和素材序号；JSON 或 schema 校验失败时在同一任务内最多自动纠正 2 轮，耗尽后保留输入并提示失败。
6. 校验成功后才能进入独立个人或团队高级编辑页；页面壳顶部展示步骤条，下方挂载共享编辑组件以复用标题、分享信息、页面设置、组件编排和保存能力。维护者可修改标题、扩展参数、组件顺序和素材；同会话重新生成可保留输入，重开已存草稿后重新生成则回第一步重选并填写新描述。
7. 推荐封面先进入普通本地选图/裁剪流程。首次保存由小程序初始化 `draftRevision=0` 的高级空壳并复用既有素材上传和草稿保存；标准与高级个人共用原个人保存接口，标准与高级团队共用原团队保存接口。高级首次完整保存和应用新候选必须携带 `generationTaskId`，草稿写入与任务应用状态在同一事务提交；已有草稿手工保存不携带。上传或保存失败保留候选、任务、空壳 ID 和已上传素材，支持复用重试。保存后可选择预览或直接发布；保存和预览不扣积分，预览交互不创建真实业务数据。
8. 返回维护页点击发布；发布前展示预计消耗 20 积分并校验余额，发布成功后切换访客可访问版本并扣除一次积分。
...
1. 用户或访客触发积分消耗动作。
2. 系统识别动作类型和扣费账户：上传作品扣上传者账户；新建团队扣创建者账户；个人或团队作品集仅在发布成功时扣实际发布者对应账户；访客访问个人作品集扣个人作品集所属维护者账户；访客访问团队作品集不扣费。
3. 若访客从团队作品集跳转到个人作品集，进入个人作品集后再按个人作品集规则识别扣费账户。
4. 维护者预览页内的查看、播放、档期查询、二维码点击不进入扣费流程。
5. 系统为本次动作生成或接收唯一幂等键，并校验是否已经处理。
6. 维护端消耗动作在执行前校验余额；访客端已打开页面后的计费动作允许本次扣费将余额扣为负数。
7. 系统以原子方式更新业务记录、滚动扣费窗口、积分账户和积分流水；访客扣费后余额不足时仍提交本次动作并继续生成低余额消息和维护任务，真实计费故障时整体回滚。
8. 维护端余额不足时阻断动作并引导充值。
9. 访客下次刷新需扣费的个人作品集时，若余额不足则内容区整体加灰色磨砂遮罩，并分两行明文提示“UNDER MAINTENANCE / 维护中”。
...
5. 后端加密手机和微信，在同一事务内创建 ContactLead、关联 VisitRecord，并写入 submit_contact 行为事件；新版接口创建新线索后按独立两小时窗口扣除 10 积分。
6. 幂等命中既有线索时直接返回既有结果，不写入重复业务数据，也不发起扣费。
7. 创建成功后访客页展示成功状态；失败时返回可理解的原因并保留已填写内容供重试。
8. 有权限的维护者在线索或访问记录详情中查看脱敏摘要，进入详情时再次鉴权后查看完整联系方式并更新跟进状态。
...
- 团队不设积分账户；团队作品集发布积分由实际执行发布动作的维护者承担，编辑、保存和预览不产生作品集发布积分。
...
- 图片需使用压缩图和 CDN。
- 视频默认展示封面，点击后播放。
- 个人作品集访问记录、滚动扣费窗口和积分扣减必须在同一事务内保持一致；接口与页面应通过精简查询和索引满足首屏性能目标。
...
| 5 | 标准个人作品集 | 分享信息配置、固定组件(轮播图→档期查询→作品列表→联系信息→微信二维码)、维护页/预览页/访客页三态、草稿/发布版本管理、保存草稿不扣分/成功发布扣10积分、发布后分享 |
| 6 | 标准团队作品集 | 分享信息配置、固定组件(轮播图→团队档期查询→成员作品集→联系信息→微信二维码)、拥有者/管理者维护发布、普通成员仅预览分享、保存草稿不扣分/成功发布扣10积分、团队访问无积分消耗 |
| 7 | 团队管理 | 团队列表+创建(扣2000积分)、三级角色权限(拥有者/管理者/普通成员)、唯一码搜索添加成员、角色权限气泡说明、内容引用权限设置、邀请状态管理、所有权转移、成员退出引用失效处理 |
...
| # | 模块 | 功能点摘要 |
|---|------|-----------|
| 15 | 高级个人作品集 | 单一作品集标题、素材选择+序号分配、自然语言布局描述(含视觉风格)、单用户单活动AI任务、AI生成结构化schema(白名单校验)、JSON校验失败同任务自动纠正、第3步使用独立页面壳并复用标准编辑组件、扩展参数手工配置、组件排序+替换素材、联系方式组件可选、可选预览、访客浏览+联系信息提交、仅发布成功扣20积分 |
| 16 | 高级团队作品集 | 单一作品集标题、素材选择(成员作品+成员个人作品集)、自然语言+AI生成、单用户单活动AI任务、JSON校验失败同任务自动纠正、第3步使用独立页面壳并复用标准团队编辑组件、拥有者/管理者维护发布、普通成员仅预览分享、可选预览、仅发布成功扣20积分 |

### 14.4 三期：支付 + 完整登录（2 个模块，约 13 项功能点）

| # | 模块 | 功能点摘要 |
|---|------|-----------|
| 17 | 微信支付积分充值 | 充值页四档位(5元/10元/50元/100元)、充值确认(支付金额/基础积分/赠送积分/总到账积分)、微信支付下单+回调验签+幂等到账、原子更新订单+账户+流水、失败不增加积分、充值积分不可提现、低余额充值引导 |
...
| 音频作品 | `{uniqueCode}/work/audio/{uniqueCode}-S-{uploadTimestamp}-{uploadSequence}.{ext}` | `WFA3B1E7A2/work/audio/WFA3B1E7A2-S-1789000000000-1.mp3` |
...
- 新用户注册同时创建音频目录；初始化失败阻断注册，沿用原有行为；历史用户由既有异步目录修复任务补齐
